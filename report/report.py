...
def markdown(plan, testsuites, testcases, md_path='markdown'):
    ...
    home_message = f'- {icon} <font color=#9D9D9D size=2>{tm(plan["start_timestamp"])} - {tm(plan["end_timestamp"])}</font> 测试计划「[{plan["plan"]}](new/{plan["plan"]})」执行完成，测试结果：{result}，成功：{sc}，失败：{fc}，阻塞：{bc}\n\n'
    plan_message = f'- {icon} <font color=#9D9D9D size=2>{tm(plan["start_timestamp"])} - {tm(plan["end_timestamp"])}</font> 测试计划「[{plan["plan"]}]({plan["plan"]}/{plan["plan"]}_{tm(plan["start_timestamp"], "_")})」执行完成，测试结果：{result}，成功：{sc}，失败：{fc}，阻塞：{bc}\n\n'
    ...
    with open(p / f'README.md', 'w', encoding='UTF-8') as f:      
        f.write(home_message + txt)
    with open(new / f'{g.plan_name}.md','w', encoding='UTF-8') as f:
        f.write(md)
